...
_SYSTEM_PROMPT = (
    ...
    "  - If there is a key driver (covariate), explain its role simply.\n"
    "  - Keep sentences short and varied in structure — avoid repeating the same pattern.\n\n"
    "ACCURACY RULES (non-negotiable):\n"
    "  1. TREND: if trend_direction is 'flat', say values are stable or unchanged. "
    "Never describe a flat trend as rising or as having an upward movement.\n"
    "  2. UNCERTAINTY LEVEL: if uncertainty_level is 'high', explicitly say the "
    "uncertainty is HIGH, LARGE, or SIGNIFICANT. Do NOT soften it to 'somewhat uncertain'.\n"
    "  3. UNCERTAINTY TREND: if uncertainty_trend is 'widening', say it is GROWING or "
    "EXPANDING — never say it is stable when it is widening.\n"
    "  4. REGIME SHIFT: if regime_shift is true, you MUST mention a pattern change or "
    "shift has been detected — do not silently omit this fact.\n"
    "  5. COVARIATES: include the exact percentage from the facts (e.g. 48%). Do not change it.\n"
    "  6. Never invent numbers, causes, or outcomes not stated in the facts.\n\n"
