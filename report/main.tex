...
Exstension 1 addresses two complementary challenges: generating natural-language summaries that faithfully describe the content of quantile forecasts, and automatically verifying whether the generated text is consistent with the underlying numerical evidence.
...
We train a surrogate XGBoost regressor \cite{xgboost} to approximate the mapping from covariate features to the P50 forecast level.
Rolling windows over the covariate history produce multiple training rows with per-covariate aggregate features (mean, standard deviation, last, min, max).
SHAP TreeExplainer~\cite{shap} then attributes forecast contributions to each covariate by computing mean absolute SHAP values aggregated across the five per-covariate features.
The top-$k$ attributions (default $k=5$) are passed to the verbalizer to generate attribution-aware sentences.
...
\item \textsc{Cause}: activated when the top SHAP covariate contributes $>20\%$, expressing the driver.
...
We evaluate on the M5 forecasting competition dataset, which provides hierarchical daily sales data for Walmart stores across multiple product categories and geographic regions.
The M5 dataset offers real observed actuals against which both forecast accuracy and explanation faithfulness can be assessed.
...
The dual evaluation protocol from Section~V is applied: forecast accuracy metrics (MASE, coverage, sharpness) are computed against M5 observed actuals; explanation faithfulness metrics (NLI consistency, round-trip check) are computed against the model's own numerical features.

\textbf{Planned comparisons.}
Three verbalizer types (Template, LLM~Guided, LLM~Raw) are compared across multiple M5 product categories, with and without covariate attribution.
